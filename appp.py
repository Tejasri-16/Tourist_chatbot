...
# --- Page Configuration ---
st.set_page_config(
    page_title="India Tourist Guide 🇮🇳",
    page_icon="🇮🇳",
    layout="centered")

st.title("🇮🇳 India Tourist Guide - Your Companion for Exploring India")
st.caption("Discover amazing places to visit across India")
...
def load_data(filepath):
    """Loads the tourist place data from a CSV file."""
    try:
...
df = load_data('Tourist_places.csv')
...
tourist_places_data_string = df.to_string()
...
if prompt := st.chat_input("Where would you like to explore in India today?"):
    ...
    full_prompt = f"""
    You are 'India Tourist Guide,' a friendly and helpful conversational agent for discovering tourist places in India. Your goal is to help users discover great places to visit.

    **RULES:**
    - Use ONLY the tourist place data provided below to answer the user's question.
    - Do not make up any information or places.
    - Be friendly and conversational.
    - If the user asks a question unrelated to tourist places in India or the provided data, politely decline and steer the conversation back to exploring India.

    **TOURIST PLACE DATA:**
    {tourist_places_data_string}
